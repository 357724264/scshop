...
<body class="add-bankpadd">
			...
			<div class="tx-group" id="wx">
				...
				<div class="tx-g-list">
					...
					<div class="tx-container">
						<input type='text' class="tx-g-ipt" name="phone" value="${refund.phone}" id="phone" placeholder="请填写联系电话">
					</div>
				</div>
				<div class="tx-g-list wx">
					<span class="txtit">真实姓名&nbsp;:&nbsp;</span>
					<div class="tx-container">
						<input type='text' class="tx-g-ipt" name="realName"  value="${refund.realName}" id="realName" placeholder="请填写真实姓名">
					</div>
				</div>
				<div class="tx-g-list wx ">
					<span class="txtit">微信帐号&nbsp;:&nbsp;</span>
					<div class="tx-container">
						<input type='text' class="tx-g-ipt" name="wxAccount" value="${refund.wxAccount}" id="wxAccount" placeholder="请填写微信帐号">
					</div>
				</div>
				...
				<div class="tx-g-list bank">
					...
					<div class="tx-container">
						<a href="#" id="bankcard" data-cardid="${refund.bankcardid}" class="yhkxz"><#if refund.cardInfo>${refund.cardInfo}<#else>请选择银行卡</#if><i class="iconfont icon-xiangyou1"></i></a>
					</div>
				</div>
	...
	<div class="content">
		<div class="af-box">
			...
			<ul class="af-type">
				<li>我要退款<input type="radio" name="refundType" value="0" <#if refund.refundType==0>checked="checked"</#if>><i class="iconfont icon-dui3"></i></li>
				<li>我要退货<input type="radio" name="refundType" value="1" <#if refund.refundType==1>checked="checked"</#if>><i class="iconfont icon-dui3"></i></li>
			</ul>
			<h3 class="af-tit"><span class="bx">*</span>退款金额<span class="mark">（最高可退金额&nbsp;:&nbsp;<span>&yen;${total}</span>）</span></h3>
			<input type="number" value="${refund.total}" name="total" data-total="${total}" id="total" class="af-money" placeholder="请输入退款金额">
			
			<h3 class="af-tit"><span class="bx">*</span>退款原因<span class="mark"></span></h3>
			<textarea id="reason" class="af-detail">${refund.reason}</textarea>
	...
	<script type="text/javascript">
		...
		$(function(){
			...
			if("${bankcardid}" || "${refund.refundWay}"==1) {
			...
			$(document).on('click','#bankcard',function(){
				...
				location.href = 'bankcard!list.action?refundid=${refund.id}&orderid=${orderid}&total=${total}&bankcardid=' + bankcardid;								
			});
			...
			$(document).on('click','#applyBtn',function(){
				...
				var cardInfo = "${refund.cardInfo}";
				var	bankcardid = "${refund.bankcardid}";
