...
//	private OrderQuartService orderQuartService;
	...
	public String comment() {
		...
		for (OrderItem item : items) {
			...
			if (itemjsonObject != null) {
				...
				comment.setShopid(item.getShopid());
				comment.setProductName(item.getName());
				int clevel = 3;
				if (point >= 2 && point <= 3) {
					clevel = 2;
				} else if (point < 2) {
	...
	public String create() {
		...
		for (int i = 0; i < size; i++) {
			...
			System.out.println("subproduct shopid===" + subProduct.getShopid());
			System.out.println("subproudctid == " + subProduct.getId());
			// 要判断商品是不是属于该商店
