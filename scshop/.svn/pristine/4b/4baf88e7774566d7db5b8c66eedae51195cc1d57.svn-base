...
<dd >
	<div class="qrdd-odrbox">
		...
		<div class="qrdd-pro-box">
			...
			<#assign num = num+1 />
			<a href="#" onclick="openWin('order','${order.id}');" class='external qrdd-prolist'>
	...
	<div class="ord-btnbox">
		...
		<a class="external" href="#" onclick="openWin('order_expressage','${order.id}');" >查看物流</a>
		</#if>
		<#if order.payStat==0>			  			
		...
		<#if order.payStat==3>			  			
		<a class="external" href="#" onclick="openWin('order_comment','${order.id}');" >订单评论</a>
		</#if>					  			
	</div>
</dd>
</#list>	  					