...
<dd >
	...
	<div class="ord-btnbox">
		...
		<a class="external" href="refund!apply.action?orderid=${order.id}&total=${order.realmoney}" >申请退款</a>
		</#if>
		<#if order.payStat==6>
		<a class="external" href="refund!edit.action?orderid=${order.id}&total=${order.realmoney}" >修改退款信息</a>
		</#if>
	</div>
</dd>
</#list>	  					